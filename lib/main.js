...
export function activate () {
  console.log('loaded dev version')
  ...
  disposables.add(atom.commands.add('atom-text-editor', {
    ...
    'line-ending-selector:convert-to-CR': (event) => {
      const editorElement = event.target.closest('atom-text-editor')
      setLineEnding(editorElement.getModel(), '\r')
    }
  }))
}
...
function getLineEndings (buffer) {
  if (typeof buffer.find === 'function') {
    return Promise.all([
      ...
      buffer.find(CRRegExp)
    ]).then(([hasLF, hasCRLF, hasCR]) => {
      const result = new Set()
      if (hasLF) result.add('\n')
      if (hasCRLF) result.add('\r\n')
      if (hasCR) result.add('\r')
      return result
    })
  } else {
